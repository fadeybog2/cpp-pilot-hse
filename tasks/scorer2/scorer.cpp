...
void Scorer::OnCheckFailed(const StudentName &student_name, const TaskName &task_name) {
    student_tasks[student_name][task_name].check_success = false;
    if (student_tasks[student_name][task_name].merge_closed) {
        student_tasks[student_name].erase(task_name);
        if (student_tasks[student_name].empty()) {
            student_tasks.erase(student_name);
        }
    }
}
void Scorer::OnCheckSuccess(const StudentName &student_name, const TaskName &task_name) {
    student_tasks[student_name][task_name].check_success = true;
}
void Scorer::OnMergeRequestOpen(const StudentName &student_name, const TaskName &task_name) {
    student_tasks[student_name][task_name].merge_closed = false;
}
void Scorer::OnMergeRequestClosed(const StudentName &student_name, const TaskName &task_name) {
    student_tasks[student_name][task_name].merge_closed = true;
    if (!student_tasks[student_name][task_name].check_success) {
        student_tasks[student_name].erase(task_name);
        if (student_tasks[student_name].empty()) {
            student_tasks.erase(student_name);
        }
...
ScoreTable Scorer::GetScoreTable() const {
    ...
    for (const auto &[student_name, tasks] : student_tasks) {
